...
import { getCfrTitle22 } from "../api";

export default function CfrTitle22() {
  ...
  async function loadPackages() {
    console.log("Loading CFR Title 22 packages...");
    setIsLoading(true);
    setError(null);

    try {
      const data = await getCfrTitle22();
      ...
      console.error("Failed to load CFR Title 22 packages", err);
      setError(err.message || "Unable to load CFR Title 22 data.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function togglePackage(packageId) {
    ...
    if (!packageDetails[packageId] && !detailsLoading[packageId]) {
      ...
      try {
        const data = await getCfrTitle22({ packageId });
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px" }}>
          <div>
            ...
            <p style={{ margin: "4px 0 0", fontSize: "12px", color: "#4b5563" }}>
              Browse the Code of Federal Regulations Title 22 volumes and sections via the GovInfo CFR API.
            </p>
          </div>
          <button
