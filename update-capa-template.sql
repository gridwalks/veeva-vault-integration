...
-- First, let's check what workflow templates exist
...
-- Update existing CAPA template or create new one if none exists
...
-- If no CAPA template was updated (no rows affected), create a new one
INSERT INTO qms_chat_workflow_templates (name, description, is_active, document_template, created_at, updated_at)
SELECT 'CAPA Workflow', 
       'Comprehensive Corrective and Preventive Action workflow with 25 structured questions',
       true,
       'CORRECTIVE AND PREVENTIVE ACTION (CAPA) DOCUMENT
...
This CAPA document has been generated through a structured workflow process to ensure comprehensive investigation and documentation of corrective and preventive actions in accordance with quality management system requirements.',
       ...
       CURRENT_TIMESTAMP
WHERE NOT EXISTS (
    SELECT 1 FROM qms_chat_workflow_templates WHERE name ILIKE '%CAPA%'
)
RETURNING id, name, is_active;
